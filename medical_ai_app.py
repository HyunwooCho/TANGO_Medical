...
def run_streamlit():
    ...
    def update_num_classes():
        st.session_state["num_classes"] = st.session_state["num_classes_from_dataset"]
    ...
    with tabs[0]:
        ...
        with col1:
            ...
            if data_source == "샘플 데이터 생성":
                ...
                if st.button("데이터셋 다운로드 및 준비"):
                    with st.spinner(f"{selected_dataset} 데이터셋을 다운로드하고 준비 중입니다..."):
                        try:
                            ...
                            # 데이터 정보 저장
                            st.session_state.data_name = selected_dataset
                ...
                #     st.write(f"**데이터셋 이름:** `{st.session_state.data_name}`")
                #     st.write(f"**총 데이터 개수:** `{st.session_state.data_count}` 개")
                #     st.write(f"**클래스 수:** `{st.session_state.num_classes}` 개")
        ...
        with col2:
            ...
            if 'trainer' in st.session_state:
                model_info = st.session_state.trainer.get_args()
                left, right = st.columns([1,1])

                # Left column for model information
                with left:
                    st.markdown(
                        f"""
                        <div style="background-color: #F0F8FF; padding: 20px; border-radius: 10px; box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.1);">
                            <h3 style="color: #1E90FF;">모델: <span style="color: #4682B4;">{model_info['model_type']}</span></h3>
                            <div style="display: inline-block; background-color: #E6F7FF; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">학습률: <span style="color: #4682B4; font-size: 1.5em;">{model_info['learning_rate']}</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #E6F7FF; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">디바이스: <span style="color: #4682B4; font-size: 1.5em;">{model_info['device']}</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #E6F7FF; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">옵티마이저: <span style="color: #4682B4; font-size: 1.5em;">{model_info['optim']}</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #E6F7FF; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">손실함수: <span style="color: #4682B4; font-size: 1.5em;">{model_info['loss_func']}</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #E6F7FF; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">검증메트릭: <span style="color: #4682B4; font-size: 1.5em;">{model_info['metric']}</span></p>
                            </div>
                        </div>
                        <br>
                ...
                # Right column for dataset information
                with right:
                    st.markdown(
                        f"""
                        <div style="background-color: #FFF0F5; padding: 20px; border-radius: 10px; box-shadow: 2px 2px 5px rgba(0, 0, 0, 0.1);">
                            <h3 style="color: #FF1493;">데이터셋: <span style="color: #8B008B;">{st.session_state.data_name}</span></h3>
                            <div style="display: inline-block; background-color: #FDE2E4; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">총 데이터 개수: <span style="color: #8B008B; font-size: 1.5em;">{st.session_state.data_count} 개</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #FDE2E4; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">학습용 데이터: <span style="color: #8B008B; font-size: 1.5em;">{model_info['train_data_count']} 개</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #FDE2E4; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">검증용 데이터: <span style="color: #8B008B; font-size: 1.5em;">{model_info['val_data_count']} 개</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #FDE2E4; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">배치 크기: <span style="color: #8B008B; font-size: 1.5em;">{model_info['batch_size']}</span></p>
                            </div>
                            <div style="display: inline-block; background-color: #FDE2E4; padding: 15px; margin: 5px; border-radius: 8px; width: auto;">
                                <p style="color: #333;">클래스 수: <span style="color: #8B008B; font-size: 1.5em;">{model_info['num_classes']} 개</span></p>
                            </div>
                        </div>
                        <br>
                        """, unsafe_allow_html=True
                ...
                if not st.session_state.training_in_progress:
                    start_button = st.button("모델 학습 시작")
                    end_button = False
                else:
                    start_button = False
                    end_button = st.button("모델 학습 중지")

                if 'start_button' in locals() and start_button:
                    if st.session_state.trainer is None:
                        st.error("trainer가 초기화되지 않았습니다.")
                        return
                    ...
                    async def update_progress(metrics):
                        try:
                            ...
                            st.error(f"진행 상화 업데이트 중 오류 발생: {e}")
